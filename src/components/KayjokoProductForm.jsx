...
import { categories, getSubcategoriesByCategory, fetchAndUpdateSubcategories } from '@/data/categoriesData';
...
const KayjokoProductForm = ({ isOpen, onClose, productToEdit, onSave }) => {
  ...
  useEffect(() => {
    if (formData.category) {
      const subs = getSubcategoriesByCategory(formData.category);
  ...
  useEffect(() => {
    if (productToEdit) {
      setFormData({
        ...
        category: productToEdit.category || productToEdit.category_id || '', 
        subcategory: productToEdit.subcategory || productToEdit.subcategory_id || '',
  ...
  const validate = () => {
    ...
    if (!isCreatingSubcategory && !formData.subcategory) newErrors.subcategory = "La sous-catégorie est requise";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let finalSubcategoryId = formData.subcategory;
      ...
      if (isCreatingSubcategory) {
        ...
        const { data: newSubData, error: subErr } = await supabase
          .from('kayjoko_subcategories')
          .insert({ category_id: catId, name: newSubcategoryName.trim(), description: '' })
          .select()
          .single();
        ...
        // Refresh global categories to include the new one
                      ...
                      {!isCreatingSubcategory ? (
                        <>
                          <select 
                            ...
                            className={`w-full p-2.5 rounded-lg border ${errors.subcategory ? 'border-red-500' : 'border-gray-300'} focus:ring-2 focus:ring-[#003D7A] outline-none bg-white`}
                            disabled={!formData.category || activeSubcategories.length === 0}
                          >
                            <option value="">
