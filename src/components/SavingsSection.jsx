...
function SavingsSection() {
  ...
  const discountedProducts = products.filter(p => p.is_promo).slice(0, 4);
       ...
       <section className="py-12 bg-white text-center">
         <div className="max-w-7xl mx-auto px-6">
            <h2 className="text-[18px] font-bold text-[#003D7A] mb-4 text-center uppercase">
                ÉCONOMISEZ L'ARGENT AVEC KAYJOKKO
            </h2>
            <p className="text-gray-400 text-sm">Aucune offre spéciale disponible actuellement.</p>
         </div>
       </section>
    );
  }
          ...
          {discountedProducts.map((product, idx) => (
            ...
            >
              <div className="relative h-[180px] w-full bg-white flex items-center justify-center overflow-hidden">
                ...
                />
                <div className="absolute top-0 left-0 bg-[#4CAF50] text-white text-[11px] font-bold px-2 py-1 rounded-br-lg shadow-sm">
                  {product.badge || 'ÉCONOMISEZ'}
                </div>
              </div>
              ...
              <div className="p-4 flex flex-col">
                ...
                <div className="flex items-center gap-2 mb-3">
                   {product.originalPrice && (
                     <span className="text-[11px] text-[#999] line-through">{product.originalPrice.toLocaleString()} F</span>
                   )}
